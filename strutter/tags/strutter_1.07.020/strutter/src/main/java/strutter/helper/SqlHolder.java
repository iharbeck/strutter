...
public class SqlHolder
{
	 ...
	 * 
	 * public String doLocalize(String content) { StringBuffer buf = new
	 * StringBuffer(); Matcher m = patternLocalize.matcher(content);
	 * 
	 * MessageResources mr =
	 * (MessageResources)ActionHelper.getRequestAttribute(Globals.MESSAGES_KEY);
	 * Locale locale =
	 * (Locale)ActionHelper.getSessionAttribute(Globals.LOCALE_KEY);
	 * 
	 * int start=0; while ( m.find() ) { String alias =
	 * content.substring(m.start()+1, m.end());
	 * 
	 * buf.append(content.substring(start, m.start()));
	 * buf.append(getMessage(alias, mr, locale));
	 * 
	 * start = m.end(); } buf.append(content.substring(start));
	 * 
	 * return buf.toString(); }
	 * 
	 * private final String getMessage(String key, MessageResources mr, Locale
	 * locale) { if(mr == null) return ("XXX" + key + "XXX").substring(0,
	 * Math.min(30, key.length())); String msg = mr.getMessage(locale,
	 * key).replace('?', 'X'); return msg.substring(0, Math.min(30,
	 * msg.length())); }
	 */
}
