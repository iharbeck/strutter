...
//
// $URL: https://htmlparser.svn.sourceforge.net/svnroot/htmlparser/trunk/lexer/src/main/java/org/htmlparser/lexer/Lexer.java $
	        ...
	        ParserException
	{
				...
				case 1: // prior to the optional qualifier
					switch(ch)
					{
						...
						case '@': // <%@
							...
							attributes.addElement(new TagAttribute(mPage, start + 1, code, -1, -1, (char)0));
							state = 2;
							break;
						default: // <%x
		...
		if(4 == state) // normal exit
		{
			if(0 != code)
			{
				...
				attributes.addElement(new TagAttribute(mPage, code, state, -1, -1, (char)0));
