...
import strutter.optional.controller.tags.ConfigInterface;
...
public class ActionPlugin implements PlugIn
{
	...
	boolean registAction(ModuleConfig moduleConfig, Class actionClass)
	{
		...
		try {
			...
			if(isInterfaceImplementer(actionClass, BaseInterface.class)
			 || isInterfaceImplementer(actionClass, DirectInterface.class)
			 || isInterfaceImplementer(actionClass, ConfigInterface.class))
			{
				conf = new ActionConfig();

				if(isInterfaceImplementer(actionClass, ConfigInterface.class))
				{
					((ConfigInterface)actionClass.newInstance()).config(conf);
				}
			}
			else
			{
...
//				}
			...
			for(int i=0; i < forwards.length; i++)
			{
				...
				aforward.setPath(views + forward.getPath());
			...
			if(forwards.length == 0)
			{
				...
				{
					String file = views + path + "_" + defaultpages[i] + ".jsp";
